...
def viz(recon: torch.Tensor, target: torch.Tensor, loss_trace: float):
    fig, (ax1, ax2, ax3) = plt.subplots(ncols=3)
    ax1.imshow(target.detach().cpu().numpy().squeeze(), cmap="gray", vmin=0, vmax=1.0)
    ax2.imshow(recon.detach().cpu().numpy().squeeze() , cmap="gray", vmin=0, vmax=1.0)
    ax3.plot(loss_trace)
...
def train(dataset: RawImgDataset, num_epochs: int, lr: float = 1e-4, save_path = Path("."), checkpoint: dict = None):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # viz(recon[0], batch_imgs[0], loss_trace)
...
if __name__ == "__main__":
    dataset_path = Path(r'C:%USERPROFILE%\Workspace\Zurich-RAW-to-DSLR-Dataset\train\huawei_raw')
    ...
    encoder_model = None #  Path("./model_data/encoder_epoch_3.pt")
    decoder_model = None #  Path("./model_data/decoder_epoch_3.pt")
